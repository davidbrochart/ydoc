...
class YText(YType):
    ...
    def insert(
        ...
        index: int,
        text: str,
        transaction_origin: Any | None = None,
        ...
        def insert_in_transaction(txn: "Transaction") -> None:
            ...
            for i, char in enumerate(text):
                ...
                char_id = create_id(
                    self._doc.client_id,
                    txn.doc.store.get_state(self._doc.client_id) + i,
                )

                # Create item with the character content
                item = Item(
                    id=char_id, content=[char], parent=self._item, parent_sub=self._name
                )
    ...
    def delete(
        self, index: int, length: int, transaction_origin: Any | None = None
    ) -> None:
    ...
    def set(self, key: str, value: Any, transaction_origin: Any | None = None) -> None:
        ...
        def set_in_transaction(txn: "Transaction") -> None:
            ...
            if key in self._map:
                ...
                # Create new item
                item_id = create_id(
                    self._doc.client_id, txn.doc.store.get_state(self._doc.client_id)
                )
                item = Item(
                    id=item_id, content=[key, value], parent=self._item, parent_sub=key
                )
                self._doc.store.add_struct(item)
                self._map[key] = item
...
class YArray(YType):
    ...
    def insert(
        self, index: int, content: List[Any], transaction_origin: Any | None = None
    ) -> None:
        ...
        def insert_in_transaction(txn: "Transaction") -> None:
            ...
            for i, element in enumerate(content):
                element_id = create_id(
                    self._doc.client_id,
                    txn.doc.store.get_state(self._doc.client_id) + i,
                )
                item = Item(id=element_id, content=[element], parent=self._item)
                self._doc.store.add_struct(item)
    ...
    def delete(
        self, index: int, length: int = 1, transaction_origin: Any | None = None
    ) -> None:
    ...
    def __setitem__(
        self, index: int, value: Any, transaction_origin: Any | None = None
    ) -> None:
...
class YXml(YType):
    ...
    def set_attribute(
        self, name: str, value: str, transaction_origin: Any | None = None
    ) -> None:
    ...
    def to_json(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "children": [
                child.to_json() if hasattr(child, "to_json") else child
...
def create_y_type(type_name: str, name: str | None = None, *args, **kwargs) -> YType:
    ...
    type_map = {"text": YText, "map": YMap, "array": YArray, "xml": YXml}
